...
export default function QueryRaised() {
  ...
  const [actionType, setActionType] = useState<'approve' | 'deferral' | 'otc' | 'revert' | 'request-approve' | 'request-deferral' | 'request-otc' | null>(null);
  ...
  // Management users for approval requests
  const [managementUsers, setManagementUsers] = useState<Array<{
    managementId: string;
    name: string;
    canApproveGeneral: boolean;
    canApproveOTC: boolean;
    canApproveDeferral: boolean;
  }>>([]);
  
  // Available people for actions - Management team members only
  const availablePeople: string[] = managementUsers.map(user => user.name);
  
  // Fetch management users
  const fetchManagementUsers = useCallback(async () => {
    try {
      const response = await fetch('/api/management/users?active=true');
      const result = await response.json();
      
      if (result.success) {
        setManagementUsers(result.data || []);
        console.log('✅ Management users loaded:', result.data?.length || 0);
      } else {
        console.warn('⚠️ Failed to load management users');
      }
    } catch (error) {
  ...
  // Load management users on component mount
  useEffect(() => {
    ...
    onSuccess: (data, variables) => {
      ...
      switch (variables.action) {
        ...
        case 'request-approve':
          message = `Approval request sent to Management team successfully! 📤`;
          break;
        case 'request-deferral':
          message = `Deferral request sent to Management team successfully! 📤`;
          break;
        case 'request-otc':
          message = `OTC request sent to Management team successfully! 📤`;
          break;
      }
  ...
  // Action handlers
  const handleAction = (type: 'approve' | 'deferral' | 'otc' | 'request-approve' | 'request-deferral' | 'request-otc') => {
    setActionType(type);
  ...
  const loadChatMessages = async (queryId: number) => {
    try {
      ...
      // First, find the query to determine which teams it's marked for
      const queryDetails = individualQueries.find(q => q.id === queryId || q.queryId === queryId.toString());
      const markedForTeam = queryDetails?.markedForTeam || 'both';
      
      console.log(`📋 Query ${queryId} is marked for: ${markedForTeam}`);
      
      // Fetch messages from all relevant teams based on the query's marked teams
      // eslint-disable-next-line prefer-const
      let allMessages: any[] = [];
      
      // Always get Operations messages and system messages
      const operationsResponse = await fetch(`/api/query-actions?queryId=${queryId}&type=messages`);
      const operationsResult = await operationsResponse.json();
      
      if (operationsResult.success) {
        const operationsMessages = operationsResult.data || [];
        allMessages.push(...operationsMessages);
        console.log(`🏢 Loaded ${operationsMessages.length} operations/system messages`);
      }
      
      // Get Sales messages if marked for sales or both teams
      if (markedForTeam === 'sales' || markedForTeam === 'both') {
        try {
          const salesResponse = await fetch(`/api/query-actions?queryId=${queryId}&type=messages&team=Sales`);
          const salesResult = await salesResponse.json();
          
          if (salesResult.success) {
            const salesMessages = salesResult.data || [];
            // Filter out duplicates and add only Sales team messages
            const uniqueSalesMessages = salesMessages.filter((msg: any) => 
              msg.team === 'Sales' && !allMessages.some(existing => existing.id === msg.id)
            );
            allMessages.push(...uniqueSalesMessages);
            console.log(`💼 Loaded ${uniqueSalesMessages.length} Sales team messages`);
          }
        } catch (error) {
          console.warn('Could not load Sales messages:', error);
        }
      }
      
      // Get Credit messages if marked for credit or both teams
      if (markedForTeam === 'credit' || markedForTeam === 'both') {
        try {
          const creditResponse = await fetch(`/api/query-actions?queryId=${queryId}&type=messages&team=Credit`);
          const creditResult = await creditResponse.json();
          
          if (creditResult.success) {
            const creditMessages = creditResult.data || [];
            // Filter out duplicates and add only Credit team messages
            const uniqueCreditMessages = creditMessages.filter((msg: any) => 
              msg.team === 'Credit' && !allMessages.some(existing => existing.id === msg.id)
            );
            allMessages.push(...uniqueCreditMessages);
            console.log(`💳 Loaded ${uniqueCreditMessages.length} Credit team messages`);
          }
        } catch (error) {
          console.warn('Could not load Credit messages:', error);
        }
      }
      ...
      uniqueMessages.sort((a: { timestamp: string }, b: { timestamp: string }) => 
        new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()
      );
      
      console.log(`📬 Total unique messages loaded: ${uniqueMessages.length} for query ${queryId}`);
      console.log(`📊 Message breakdown: Operations/System + Sales: ${allMessages.filter(m => m.team === 'Sales').length} + Credit: ${allMessages.filter(m => m.team === 'Credit').length}`);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
            <h3 className="text-lg font-bold mb-4 text-black">
              ...
              {actionType === 'request-approve' && 'Request Approval'}
              {actionType === 'request-deferral' && 'Request Deferral'}
              {actionType === 'request-otc' && 'Request OTC'}
            </h3>
            
            <div className="space-y-4">
              {(actionType === 'deferral' || actionType === 'otc' || actionType === 'request-deferral' || actionType === 'request-otc') && (
                <div>

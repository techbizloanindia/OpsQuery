...
export interface QueryChatModalProps {
  ...
  readOnly?: boolean; // New prop for management read-only access
}
...
export default function QueryChatModal({
  ...
  createdAt,
  status = 'pending',
  readOnly = false // Default to false for normal operation
}: QueryChatModalProps) {
  ...
  return (
    <div className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 ${isOpen ? '' : 'hidden'}`}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl h-[85vh] flex flex-col">
        ...
        {/* Message Input - Hide if read-only */}
        {!readOnly && (
        ...
        {/* Read-only Notice for Management */}
        {readOnly && (
          <div className="p-4 border-t border-gray-200 bg-indigo-50">
            <div className="flex items-center gap-2 text-indigo-700">
              <FiEdit3 className="w-5 h-5" />
              <span className="font-medium">Management View - Read Only</span>
            </div>
            <p className="text-sm text-indigo-600 mt-1">
              You can view conversations but cannot send messages. Use the "Approval Requests" tab to approve, defer, or OTC with remarks.
            </p>
          </div>
        )}
      </div>

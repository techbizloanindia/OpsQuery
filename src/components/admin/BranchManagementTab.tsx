...
export default function BranchManagementTab() {
  ...
  const validateForm = () => {
    ...
    } else if (branches.some(branch => branch.branchCode.toLowerCase() === formData.code.toLowerCase())) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        showSuccess("Branch Created", `Branch "${formData.name}" created successfully!`);
        // Reset form
        setFormData({
          name: '',
          code: '',
        ...
        showError("Creation Failed", `Failed to create branch: ${result.error}`);
      }
    } catch (error) {
      console.error('Error creating branch:', error);
      showError("System Error", 'Failed to create branch. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleBranchStatus = async (branchId: string, currentStatus: boolean) => {
    try {
      ...
      if (result.success) {
        ...
        alert(`Failed to update branch status: ${result.error}`);
      }
    } catch (error) {

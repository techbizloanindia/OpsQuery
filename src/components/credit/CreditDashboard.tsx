 ...
 * 
 * @fileoverview Credit Dashboard - Main interface for Credit team with real-time messaging
...
// Add the same tab navigation logic to Credit Dashboard as Sales Dashboard
...
// Fetch queries assigned to Credit team
const fetchCreditQueries = async (): Promise<Query[]> => {
  const response = await fetch('/api/queries?team=credit');
    const result = await response.json();
    
    if (!response.ok || !result.success) {
      throw new Error(result.error || 'Failed to fetch credit queries');
    }
...
export default function CreditDashboard() {
  ...
  const fetchCreditQueryStats = async () => {
    try {
      const response = await fetch('/api/queries?stats=true&team=credit');
      const result = await response.json();
      ...
      console.error('Error fetching credit query stats:', error);
    }
  };

  // Initial load
  useEffect(() => {
    fetchCreditQueryStats();
  }, []);
  ...
  // Fetch Credit queries with real-time updates
  const { data: queries, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['creditQueries'],
    queryFn: fetchCreditQueries,
  ...
  const individualQueries = React.useMemo(() => {
    ...
    queries.forEach(queryGroup => {
      // Only include queries marked for credit team
      if (queryGroup.markedForTeam === 'credit' || queryGroup.markedForTeam === 'both') {
        ...
        .catch(error => {
          ...
          setHistoryData([
            ...
            },
            {
              ...
              time: new Date(Date.now() - 3 * 60 * 60 * 1000).toLocaleString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
                hour: '2-digit',
              ...
              text: selectedQuery.queries[0]?.text || 'Please verify credit score and assess lending eligibility.',
              sender: selectedQuery.submittedBy,
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async (data: { queryId: number; message: string }) => {
      console.log(`💬 Sending message from Credit team for query ${data.queryId}`);
      
      const requestBody = {
        type: 'message',
        queryId: data.queryId,
        message: data.message,
        addedBy: user?.name || 'Credit User',
        team: 'Credit'
      };
  ...
  if (isLoading && !queries) {
    return <LoadingState message="Loading credit queries..." />;
  }
  
  if (isError && !queries) {
    return <ErrorState message={error?.message || 'Failed to load credit queries'} onRetry={refetch} />;
  }
            ...
            <div className="flex items-center justify-between">
              <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-sm text-gray-600">Credit Dashboard</span>
          ...
          {/* Applications List */}
              <div className="flex-grow overflow-y-auto p-4 space-y-3" style={{
                ...
                {filteredApplications.length === 0 ? (
                  <EmptyState message="No applications with queries assigned to Credit team" />
                ) : (
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <h2 className="text-md font-semibold text-gray-800">{appNo}</h2>
                            ...
                            <p className="text-xs text-gray-500">
                              {appQueries.length} individual {appQueries.length === 1 ? 'query' : 'queries'} for Credit team
                            </p>
                    </div>
          ...
          {/* Header */}
            ...
            <div>
                    <h1 className="text-xl font-bold text-gray-800">
                      Queries for <span className="text-green-600">{selectedAppNo}</span>
                    </h1>
                      ...
                      {entry.actionType === 'revert' ? (
                        <RevertMessageBox 
                          message={{
                            ...
                            sender: entry.sender || 'Credit Team',
                            senderRole: entry.senderRole || 'credit',
                            team: 'Credit Team',

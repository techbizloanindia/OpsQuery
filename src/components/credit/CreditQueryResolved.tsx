...
// Fetch resolved queries for Credit team
const fetchCreditResolvedQueries = async (): Promise<Query[]> => {
  const response = await fetch('/api/queries?team=credit&status=resolved');
  const result = await response.json();
  
  if (!response.ok || !result.success) {
    throw new Error(result.error || 'Failed to fetch Credit resolved queries');
  }
  
  return result.data.filter((query: Query) => 
    (query.sendToCredit || 
     query.sendTo.includes('Credit') || 
...
export default function CreditQueryResolved() {
  ...
  const { data: queries, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['creditResolvedQueries'],
    queryFn: fetchCreditResolvedQueries,
  ...
  const { data: queryHistory, isLoading: historyLoading } = useQuery({
    queryKey: ['creditQueryHistory', selectedQuery?.id],
  ...
  const openModal = (application: any) => {
    // Create a mock query object for the modal with application data
    const mockQuery: Query = {
      ...
      sendTo: [],
      submittedBy: application.queries[0]?.submittedBy || 'Credit Team',
  ...
  if (isLoading) {
    return <LoadingState message="Loading Credit resolved queries..." />;
  }

  if (isError) {
    return <ErrorState message={error?.message || 'Failed to load Credit resolved queries'} onRetry={refetch} />;
  }
      ...
      <div className="container mx-auto p-4 md:p-8">
          ...
          <div className="flex items-center mt-2 sm:mt-0 gap-4">
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none bg-white border border-slate-300 rounded-md py-2 pl-3 pr-8 text-sm font-medium text-black hover:border-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
              >
        ...
        <div className="mb-6">
          <input
            ...
            className="w-full max-w-md px-4 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 text-black font-bold bg-white"
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="bg-gradient-to-r from-green-600 to-emerald-600 text-white p-4">
            <h2 className="text-xl font-semibold">💳 Credit - Individual Resolved Queries</h2>
          </div>
          
          {filteredAndSortedQueries.length === 0 ? (
            <div className="p-8">
              <EmptyState message="No resolved queries found for Credit team" />
            </div>
          ) : (
                      ...
                      <td className="px-4 py-4">
                        <div className="text-sm">
                          ...
                          <div className="text-gray-600">
                            By: {query.resolvedBy || 'Credit Team'}
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        <button
                          ...
                          className="text-sm font-semibold text-green-600 hover:text-green-800 transition-colors duration-200 flex items-center gap-1 bg-green-50 px-3 py-2 rounded-lg hover:bg-green-100"
                        >
      ...
      {showModal && selectedQuery && (
        ...
        >
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col transform transition-transform duration-300">
            {/* Modal Header */}
            <div className="flex justify-between items-center p-6 border-b border-slate-200 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-t-xl">
            ...
            <div className="p-6 overflow-y-auto flex-grow">
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="space-y-3">
                  {selectedQuery.queries.map((query: any, index: number) => (
                    <div key={query.id} className="bg-white p-4 rounded-lg border border-l-4 border-l-green-500 shadow-sm">
                      <div className="flex items-start justify-between mb-2">
                        <p className="font-semibold text-green-800 flex items-center gap-2">
                          ✅ Query #{index + 1} - {query.status?.toUpperCase()}
                        </p>
                        <span className="text-xs text-black bg-green-100 px-2 py-1 rounded">
                          {formatDateTime(query.resolvedAt || selectedQuery.resolvedAt)}
                        </span>
                      </div>
                      <p className="text-sm text-black mb-3 font-medium bg-green-50 p-3 rounded">
                        "{query.text}"
                      </p>
                      <div className="text-xs text-black flex items-center gap-4">
                        <span>
                          <span className="font-medium">Resolved by:</span> 
                          <span className="ml-1">{query.resolvedBy || selectedQuery.resolvedBy || 'Credit Team'}</span>
                        </span>
                        <span>
              ...
              <div>
                <h3 className="font-semibold text-black text-lg mb-4 flex items-center gap-2">
                  ...
                  {historyLoading && <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-green-600"></div>}
                </h3>

/**
 * OpsQuery - Sales Navbar Component
 * Copyright (c) 2024 OpsQuery Development Team
 * 
 * Licensed under the MIT License.
 * 
 * @fileoverview Sales Navbar - Navigation tabs for sales dashboard
...
const SalesNavbar: React.FC<SalesNavbarProps> = ({
  activeTab,
  onTabChange,
  stats,
  onRefresh,
  ...
  useBranchUpdateListener((branches) => {
    console.log('🔄 Sales Navbar: Received branch update notification');
    fetchBranches();
  });
  ...
  const fetchBranches = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/users/branches/${user.employeeId}?team=sales&realtime=true`);
      const result = await response.json();

      if (result.success && result.data) {
        setAssignedBranches(result.data);
        console.log(`📊 Sales Navbar: Updated with ${result.count} real-time branches`);
      }
    } catch (error) {
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
          <div>
            ...
            <div className="flex items-center gap-2 text-sm text-blue-100 mt-1">
              <span>Welcome, {user?.name || user?.employeeId || 'Sales User'}</span>
              <span>•</span>
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
            ...
            <p className="text-gray-600 mb-6">Are you sure you want to logout from the Sales Dashboard?</p>
